...
pair<Mat, int> FingerCount::findFingersCount(Mat inputImage, Mat frame)
{
    Mat contours_image = Mat::zeros(inputImage.size(), CV_8UC3);

    // check if the source image is good
    if (inputImage.empty())
        return make_pair<>(contours_image, -1);
    ...
    findContours(inputImage, contours, hierarchy, RETR_EXTERNAL, CHAIN_APPROX_NONE);
    ...
    return make_pair<>(contours_image, filtered_finger_points.size());
}
